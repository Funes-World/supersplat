...
const DEFAULT_BG_CLR: Color = { r: 0.733333, g: 0.733333, b: 0.733333, a: 1 }; // #bbbbbb
...
class Params {
  ...
  get(path: string): any {
    ...
    return (
      this.resolve(this.sources, path.split(".").map(kebabize)) ??
      this.resolve(this.sources, path.split("."))
    );
  }
